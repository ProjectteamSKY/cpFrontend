...
import { CutTypeFormData, CutType } from "../../types/cutType";

interface Props {
  defaultValues?: CutType | null;
  onSubmit: (data: CutTypeFormData) => Promise<void>;
  onCancel: () => void;
}

export function CutTypeForm({ defaultValues, onSubmit, onCancel }: Props) {
  const {
    register,
    handleSubmit,
    reset,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
      {/* Name */}
      <div>
        <Label>Cut Type Name *</Label>
        <Input
          {...register("name", { required: "Cut Type name is required", minLength: { value: 2, message: "Minimum 2 characters" } })}
