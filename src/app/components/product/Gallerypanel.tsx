...
export function GalleryPanel({
  ...
  const safeIndex =
    selectedImageIndex >= 0 && selectedImageIndex < allImages.length
      ? selectedImageIndex
      : 0;

  const currentImage =
    allImages?.length > 0 ? allImages[safeIndex] : null;

  const prev = () =>
    setSelectedImageIndex(
      safeIndex > 0 ? safeIndex - 1 : allImages.length - 1
    );

  const next = () =>
    setSelectedImageIndex(
      safeIndex < allImages.length - 1 ? safeIndex + 1 : 0
    );
